...
Kaleidoscopify.prototype.setSettings = function(reset) {
	// Number of segments.
	this.segments = reset ? this.defaultSegments : Cookies.get('setting-segments') ? Cookies.get('setting-segments') : this.defaultSegments;
	
	// Rotation speed.
	this.speed = reset ? this.defaultSpeed : Cookies.get('setting-speed') ? Cookies.get('setting-speed') : this.defaultSpeed;
	
	// Line width.
	this.lineWidth = reset ? this.defaultLineWidth : Cookies.get('setting-line-width') ? Cookies.get('setting-line-width') : this.defaultLineWidth;
	
	// Whether to use color.
	this.useColor = reset ? this.defaultUseColor : Cookies.get('setting-use-color') ? (Cookies.get('setting-use-color') === 'true') : this.defaultUseColor;
	...
	$('#settings input.slider-line-width').val(this.lineWidth).prev('label').find('.value').html(this.lineWidth);
...
Kaleidoscopify.prototype.init = function() {
	...
	// Display email after a second.
	setTimeout(function() {
		$('#about-me a.email').attr('href', 'mailto:moenen@shapish.com').html('moenen@shapish.com');
		$('#about-me').removeClass('hide-contact');
	}, 1000);
};
...
Kaleidoscopify.prototype.prepKaleido = function() {
	...
	$('#kaleido .canvas-wrap').css({
		'top': $(window).height() / -2,
		'left': $(window).width() / -2,
		'width': $(window).width(),
		'height': $(window).height()
	}).find('canvas').each($.proxy(function(i, elm) {
...
Kaleidoscopify.prototype.initSettings = function() {
	...
	// Segments slider.
	$('#settings input.slider-segments').on('input', function(e) {
	...
	// Speed slider.
	$('#settings input.slider-speed').on('input', function(e) {
		$(e.target).prev('label').find('.value').html($(e.target).val());
	...
	// Line width slider.
	$('#settings input.slider-line-width').on('input', $.proxy(function(e) {
		$(e.target).prev('label').find('.value').html($(e.target).val());
	...
	// Color checkbox.
	$('#settings .cb-use-color').click($.proxy(function(e) {
		this.onChangeSetting('cb-use-color', $(e.target).is(':checked'));
	}, this));
	
	
	// Show Seams.
	$('#settings .cb-seams').click($.proxy(function(e) {
		this.onChangeSetting('cb-seams', $(e.target).is(':checked'));
	}, this));
	
	
	// Rotate checkbox.
	$('#settings .cb-rotate').click($.proxy(function(e) {
		this.onChangeSetting('cb-rotate', $(e.target).is(':checked'));
	}, this));
	
	
	// Clip checkbox.
	$('#settings .cb-clip').click($.proxy(function(e) {
		this.onChangeSetting('cb-clip', $(e.target).is(':checked'));
	}, this));
	
	
	// Show original checkbox.
	$('#settings .cb-original').click($.proxy(function(e) {
		this.onChangeSetting('cb-original', $(e.target).is(':checked'));
	}, this));
	
	
	// Clear canvas button.
	$('#settings input.btn-clear').click($.proxy(function() {
		$('#kaleido canvas').each($.proxy(function(i, elm) {
			// Clear original canvas.
			$('#master-canvas').get(0).getContext('2d').clearRect(0, 0, $(window).width(), $(window).height());
	...
	// Reset values link.
	$('#settings a.reset-values').click($.proxy(function() {
